...
// --- Enhanced AWS Pricing Endpoint with Retry & Cache ---
app.post('/api/pricing', async (req, res) => {
    const { serviceCode, filters, resourceType } = req.body;
...
async function retryWithBackoff(fn, maxRetries, delay = 1000) {
...
// Health check with AWS connection test
app.get('/health', async (req, res) => {
    try {
        // Test AWS connection
        const testCommand = new GetProductsCommand({
...
app.listen(PORT, () => {
    ...
    console.log('📊 AWS Pricing API integration active');
